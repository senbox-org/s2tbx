...
public class RpcMetadata extends GenericXmlMetadata {
    ...
    public String getMetadataProfile() {
        return getAttributeValue(Constants.PATH_VOL_METADATA_PROFILE, Constants.DIMAP_DESCRIPTION);
    }
    ...
    private void readCoefficients() {
        if (lineNumCoeffs == null) {
            ...
            for (int i = 0; i < numCoeffs; i++) {
                ...
                lineNumCoeffs[i] = Double.parseDouble(getAttributeValue(Constants.PATH_RPC_DM_LINE_NUM + idx, String.valueOf(Double.NaN)));
                lineDenCoeffs[i] = Double.parseDouble(getAttributeValue(Constants.PATH_RPC_DM_LINE_DEN + idx, String.valueOf(Double.NaN)));
                sampleNumCoeffs[i] = Double.parseDouble(getAttributeValue(Constants.PATH_RPC_DM_SAMP_NUM + idx, String.valueOf(Double.NaN)));
                sampleDenCoeffs[i] = Double.parseDouble(getAttributeValue(Constants.PATH_RPC_DM_SAMP_DEN + idx, String.valueOf(Double.NaN)));
            }
        }
    }
}
