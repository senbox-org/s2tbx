...
public class WorldViewESAarchiveProductReaderPluginTest {
    private WorldViewESAarchiveProductReaderPlugin plugIn;
    private String productsFolder = "_worldView" + File.separator;

    @Before
    public void setup() {
        assumeTrue(TestUtil.testdataAvailable());
        plugIn = new WorldViewESAarchiveProductReaderPlugin();
    }

    @Test
    public void deimosReaderIsLoaded() {
        final Iterator iterator = ProductIOPlugInManager.getInstance().getReaderPlugIns(WorldViewESAarchiveConstants.FORMAT_NAMES[0]);
        final ProductReaderPlugIn plugIn = (ProductReaderPlugIn) iterator.next();
        assertEquals(WorldViewESAarchiveProductReaderPlugin.class, plugIn.getClass());
    }
