...
import org.esa.beam.dataio.s2.L2aTileLayout;
...
public class CodeStreamUtils {
    ...
    public static L2aTileLayout getL1cTileLayout(String uri, BoxReader.Listener listener) throws URISyntaxException, IOException {
        final File file = new File(CodeStreamUtils.class.getResource(uri).toURI());
        final FileImageInputStream stream = new FileImageInputStream(file);
        BoxReader boxReader = new BoxReader(stream, file.length(), listener);
        ...
        return new L2aTileLayout((int) is.getXsiz(), (int) is.getYsiz(), (int) is.getXtsiz(), (int) is.getYtsiz(), getXNumTiles(is), getYNumTiles(is), roar.getLevels());
    }
