...
public class ProcessObserverTest {
    ...
    public void testListDir() throws Exception {
        ...
        assertTrue(handler.out.contains(".git"));
        assertTrue(handler.out.contains(".gitignore"));
        assertTrue(handler.out.contains("src"));
        assertTrue(handler.out.contains("pom.xml"));
    ...
    public void testJavaProcessOk() throws Exception {
        final String commandLine = String.format("java -cp target/test-classes %s 2 10", DummyProcessor.class.getName());
        final Process process = Runtime.getRuntime().exec(commandLine);
        final MyHandler handler = new MyHandler();
        new ProcessObserver(process).setHandler(handler).start();
        assertTrue(handler.started);
    ...
    public void testJavaProcessMissingArg() throws Exception {
        final String commandLine = String.format("java -cp target/test-classes %s 2", DummyProcessor.class.getName());
        final Process process = Runtime.getRuntime().exec(commandLine);
        final MyHandler handler = new MyHandler();
        new ProcessObserver(process).setHandler(handler).start();
        assertTrue(handler.started);
        assertEquals("", handler.out);
        assertEquals("Usage: DummyProcessor <seconds> <steps>\n", handler.err);
        assertTrue(handler.ended);
    ...
    public void testJavaProcessCancel() throws Exception {
        final String commandLine = String.format("java -cp target/test-classes %s 10 2", DummyProcessor.class.getName());
        final Process process = Runtime.getRuntime().exec(commandLine);
        final MyHandler handler = new MyHandler();
