...
//        }
    ...
    public BufferedImage openFile(File f) {
        ...
        for (int band = 0; band < bandCount; band++) {
            ...
            System.out.println(" Data Type = " + gdal.GetDataTypeName(poBand.getDataType()));
            System.out.println(" ColorInterp = " + gdal.GetColorInterpretationName(poBand.GetRasterColorInterpretation()));
            ...
            if (poBand.GetOverviewCount() > 0) {
                System.out.println("Band has " + poBand.GetOverviewCount() + " overviews.");
            }

            if (poBand.GetRasterColorTable() != null) {
                System.out.println("Band has a color table with " + poBand.GetRasterColorTable().GetCount() + " entries.");
                for (int i = 0; i < poBand.GetRasterColorTable().GetCount(); i++) {
                    System.out.println(" " + i + ": " + poBand.GetRasterColorTable().GetColorEntry(i));
                }
            }
        ...
        if (buf_type == gdalconstConstants.GDT_Byte) {
            ...
            float[][] floats = new float[bandCount][];
            for (int i = 0; i < bandCount; i++) {
                floats[i] = new float[pixels];
                bands[i].asFloatBuffer().get(floats[i]);
            }
            ...
            throw new IllegalArgumentException("Unknown data type " + buf_type + ".");
        }
        ...
        if (poBand.GetRasterColorInterpretation() == gdalconstConstants.GCI_PaletteIndex) {
            ...
            if (bandCount > 2) {
                ...
                cm = new ComponentColorModel(cs, false, false, ColorModel.OPAQUE, buffer_type);
    ...
    public static void main(String[] args) {
        GDALDisplayImageTest test = new GDALDisplayImageTest();
        if (args.length >= 1) {
