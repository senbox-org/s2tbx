...
class CloudShadowFlaggerTest {
    ...
    void flagCloudShadowAreas(float[][] sourceBands, int[] flagArray, Map<Integer, List<Integer>> potentialShadowPositions,
                              Map<Integer, List<Integer>> offsetAtPotentialShadow, int bestOffset, Mode mode) {
...
//            analysePotentialCloudShadowArea_percentiles(counter, arrayBands, arrayIndexes, mean);
            analysePotentialCloudShadowArea_clustering(counter, arrayBands, arrayIndexes, arrayOffsets);
        } else if (counter > 0) {
...
//        System.out.println("threshold = " + threshold);
    ...
    private void analysePotentialCloudShadowArea_clustering(int counter, double[][] arrayBands, int[] arrayIndexes, int[] arrayOffsets) {
        double[] band = new double[counter];
...
//        double threshold = sortedCluster.get(i) + (sortedCluster.get(i + 1) - sortedCluster.get(i)) / 2;
        ...
        for (int j = 0; j < counter; j++) {
            if (band[j] < threshold) {
                int flagIndex = arrayIndexes[j];
                if (!((flagArray[flagIndex] & PreparationMaskBand.CLOUD_SHADOW_FLAG) == PreparationMaskBand.CLOUD_SHADOW_FLAG)
                        && arrayOffsets[j] < 3*bestOffset && arrayOffsets[j]>0) {
                    flagArray[flagIndex] += PreparationMaskBand.CLOUD_SHADOW_FLAG;
                }
            }
        }
