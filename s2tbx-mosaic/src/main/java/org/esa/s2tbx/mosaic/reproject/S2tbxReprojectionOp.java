...
//                        final GeoPos centerGeoPos = ProductUtils.getCenterGeoPos(sourceProduct);
    ...
    /*private double getReprojectedProductResolution(){
        ...
        {
            if( this.reprojectedFirstProduct.getBandAt(index).getSourceImage().getModel().getImageToModelTransform(0).getScaleX() < productResolution){
                productResolution = this.reprojectedFirstProduct.getBandAt(index).getSourceImage().getModel().getImageToModelTransform(0).getScaleX();
            }
        }
    ...
    public static class Spi extends OperatorSpi {
        ...
        public Spi() {
            super(S2tbxReprojectionOp.class);
        }
    }
