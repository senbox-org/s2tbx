package org.esa.s2tbx.framework.gpf.operators.tooladapter;
...
public class ToolAdapterIO {
    ...
    public static List<File> scanForModules() throws IOException {
        ...
        modules.addAll(scanForModules(getSystemModulesPath()));
        modules.addAll(scanForModules(getUserModulesPath()));
        return modules;
    }
    ...
    public static File getUserModulesPath() {
        if (userModulePath == null) {
            String moduleFolder = System.getProperty("netbeans.user");
            if (moduleFolder != null) {
                userModulePath = new File(new File(moduleFolder, "extensions"), "adapters");
            }
            if (!userModulePath.mkdirs()) {
    ...
    private static File getSystemModulesPath() {
        if (systemModulePath == null) {
            ...
            systemModulePath = new File(new File(homeFolder, "extensions"), "adapters");
            if (!systemModulePath.mkdirs()) {
