...
public class FinalMasksTilesComputing extends AbstractImageTilesParallelComputing {
    ...
    private final IntMatrix unionMaskProduct;
    private final IntSet differenceTrimmingSet;

    public FinalMasksTilesComputing(Product differenceSegmentationProduct, IntMatrix unionMaskProduct, IntSet differenceTrimmingSet, int tileWidth, int tileHeight) {
        ...
        for (int y = tileTopY; y < tileBottomY; y++) {
            for (int x = tileLeftX; x < tileRightX; x++) {
                ...
                if (this.differenceTrimmingSet.contains(segmentationPixelValue)) {
                    int unionPixelValue = unionMaskProduct.getValueAt(y, x);
