...
if [ -z "$app_java_home" ]; then
  if [ -f "$S2TBX_HOME/.install4j/pref_jre.cfg" ]; then
    read file_jvm_home < "$S2TBX_HOME/.install4j/pref_jre.cfg"
...
if [ -z "$app_java_home" ]; then
  if [ -f "$S2TBX_HOME/.install4j/inst_jre.cfg" ]; then
    read file_jvm_home < "$S2TBX_HOME/.install4j/inst_jre.cfg"
    test_jvm "$file_jvm_home"
    if [ -z "$app_java_home" ] && [ $tested_jvm = "false" ]; then
        rm $HOME/.install4j
        test_jvm "$file_jvm_home"
