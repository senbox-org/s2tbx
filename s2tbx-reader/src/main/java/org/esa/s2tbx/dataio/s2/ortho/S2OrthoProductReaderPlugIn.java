            ...
            if (file.isFile()) {
                ...
                if(matcher.matches()) {
                    ...
                    if (S2L1CGranuleMetadataFilename.isGranuleFilename(fileName)) {
                        ...
                        if (granuleMetadataFilename != null && level.equals("L1C")) {
                    ...
                    } else if (S2L2AGranuleMetadataFilename.isGranuleFilename(fileName)) {
                        level = matcher.group(4).substring(0,3);
                        S2L2AGranuleMetadataFilename granuleMetadataFilename = S2L2AGranuleMetadataFilename.create(fileName);
                        if (granuleMetadataFilename != null && level.equals("L2A")) {
                            String tileId = granuleMetadataFilename.tileNumber;
                            String epsg = tileIdentifierToEPSG(tileId);
                            if (getEPSG()!= null && getEPSG().equalsIgnoreCase(epsg)) {
                                decodeQualification = DecodeQualification.INTENDED;
                        ...
                        if (productFilename != null) {
                            if (level.equals("L1C") || level.equals("L2A")) {
