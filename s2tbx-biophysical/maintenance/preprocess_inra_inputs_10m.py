...
AUXDATA_DIR = os.path.join(os.path.dirname(__file__), '..', 'src/main/resources/auxdata/3_0/S2B_10m')
BIOPHYSICAL_VARIABLES = ['LAI', 'FAPAR', 'FCOVER']
FILENAME_TEMPLATE = 'Algo_S2_V2.0_SL2B_10m_{variable}.xlsx'
...
def process_variable(variable):
    ...
    extract_table_to_csv(ws, os.path.join(target_dir, '%s_Normalisation' % variable), 'B6', 'C11')
    extract_table_to_csv(ws, os.path.join(target_dir, '%s_Denormalisation' % variable), 'B18', 'C18')
    ...
    # Weights
    ws = wb[u'Weights']
    extract_table_to_csv(ws, os.path.join(target_dir, '%s_Weights_Layer1_Neurons' % variable), 'B6', 'G10')
    ...
    extract_table_to_csv(ws, os.path.join(target_dir, '%s_DefinitionDomain_MinMax' % variable), 'B4', 'D5')
    extract_table_to_csv(ws, os.path.join(target_dir, '%s_DefinitionDomain_Grid' % variable), 'A10', 'C215')
